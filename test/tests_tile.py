...
class TestTiles(unittest.TestCase):
  ...
  def test_tile_dull(self):
    tile_image = np.array(Image.open("test/tiles/s-mino-line-clear.png").convert('RGB'))
    tile = Tile(tile_image);
  ...
  def test_tile_non_dull_l_mino(self):
    tile_image = np.array(Image.open("test/tiles/L-mino-1.png").convert('RGB'))
    tile = Tile(tile_image);
    self.assertFalse(tile.is_dull())

  def test_tile_non_dull_l_mino(self):
    tile_image = np.array(Image.open("test/tiles/l-mino-2.png").convert('RGB'))
    tile = Tile(tile_image);
    self.assertFalse(tile.is_dull())

  def test_tile_dull_l_mino(self):
    tile_image = np.array(Image.open("test/tiles/L-mino-bright-transition.png").convert('RGB'))
    tile = Tile(tile_image);
    self.assertTrue(tile.is_dull())

  def test_tile_non_dull_i_mino(self):
    tile_image = np.array(Image.open("test/tiles/i-mino.png").convert('RGB'))
    tile = Tile(tile_image);
    self.assertFalse(tile.is_dull())

  def test_tile_non_dull_i_mino_2(self):
    tile_image = np.array(Image.open("test/tiles/i-mino-center.png").convert('RGB'))
    tile = Tile(tile_image);
    self.assertFalse(tile.is_dull())

  def test_tile_dull_i_mino(self):
    tile_image = np.array(Image.open("test/tiles/I-mino-line-clear.png").convert('RGB'))
    tile = Tile(tile_image);
    self.assertTrue(tile.is_dull())

  def test_tile_dull_o_mino(self):
    tile_image = np.array(Image.open("test/tiles/o-mino-line-clear.png").convert('RGB'))
    tile = Tile(tile_image);
    self.assertTrue(tile.is_dull())

  def test_tile_non_dull_o_mino(self):
    tile_image = np.array(Image.open("test/tiles/o-mino.png").convert('RGB'))
    tile = Tile(tile_image);
    self.assertFalse(tile.is_dull())